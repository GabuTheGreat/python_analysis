...
summary= all_tb_inc_sorted.groupby (['country' , 'year']).perc_rate_country_population.sum() . reset_index()

#Calculating mean, median, min, max and quartiles

summary['perc_rate_country_population'].agg(['sum','max','min','median'])

summary['perc_rate_country_population'].describe (percentiles = [0.5, 0.75,0.25]).round(2)

summary['perc_rate_country_population'].plot(kind = 'hist')
